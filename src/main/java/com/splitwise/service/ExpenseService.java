...
public class ExpenseService {
    ...
    @Autowired
    public ExpenseService(ExpenseDao expenseDao, BalanceService balanceService, UserService userService, BalanceBookService balanceBookService) {
    ...
    public Expense addExpense(Expense expense) throws UserNotFoundException {
        ...
        for (int i = 0; i < borrowerList.size(); i++) {
            ...
            Balance oldBalance = balanceService.getBalanceBetweenUsers(payee, borrowerList.get(i));
            Balance newBalance = balanceService.updateBalance(payee, borrowerList.get(i), share.get(i));
            if (oldBalance == null)
                balanceBookService.updateBalanceBook(userService.getUser(payee), userService.getUser(borrowerList.get(i)), newBalance);
        }
